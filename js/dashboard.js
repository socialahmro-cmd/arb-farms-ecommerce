    ...
    if (user) {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // If status is pending, effective Marla for tier is 0 (or stay basic)
...
function renderPlantSVG(marla) {
  ...
  if (marla < 5) {
    ...
    html += `
      ...
      <animateTransform attributeName="transform" type="translate" values="0,5; 0,0; 0,5" dur="3s" repeatCount="indefinite" />
    `;
  } else if (marla >= 21 && marla < 81) {
    // Standard (Small Plant)
    html += `
